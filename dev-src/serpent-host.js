...
function preload() {
  ...
  // hosted 产物放在 YUH-Studio/build/serpent，但原生依赖仍来自内置
  // Serpent 模块（YUH-Studio/Serpent）的 node_modules；显式注入 NODE_PATH，
  // 供 Library Worker 解析 better-sqlite3、sharp 等外部依赖。不再引用
  // 工作区旁的顶层 Serpent——所有源码均在 YUH-Studio 目录下。
  const hostedDependencies = path.join(SERPENT_ROOT, "node_modules");
  const embeddedSerpentDependencies = path.join(__dirname, "..", "Serpent", "node_modules");
  const serpentDependencies = [hostedDependencies, embeddedSerpentDependencies]

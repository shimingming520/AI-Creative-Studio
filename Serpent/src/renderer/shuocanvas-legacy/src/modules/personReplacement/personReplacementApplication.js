...
function resolveMediaRef(_0x426348) {
  ...
  // hosted Electron 上传桥返回的是 Windows 绝对路径。这里必须优先保留
  // 原始磁盘路径；若先交给 normalizeLocalPath，它会按渲染器安全规则
  // 过滤掉绝对路径，随后退回 file:// URL，导致主进程 ffprobe/ffmpeg
  // 的 existsSync 无法识别，表现为“选择视频后没有反应”。
  const nativePath = normalizeText(
    _0x426348?.path || _0x426348?.displayLocalPath || _0x426348?.localPath,
  );
  if (nativePath) return nativePath;
  return normalizeText(pickResultLocalPath(_0x426348) || _0x426348?.displayUrl || _0x426348?.videoUrl || _0x426348?.imageUrl || _0x426348?.url || _0x426348?.originalUrl);
}
function resolveMediaUrl(_0x3fef07) {
  const _0x7d2756 = resolveMediaRef(_0x3fef07);
  if (_0x7d2756) {
    // Hosted 模式的本地媒体引用是 Windows 绝对路径；浏览器播放/预览
    // 需要 file:// URL，不能直接把 C:\\... 当作 src。
    if (/^[a-zA-Z]:[\\/]/.test(_0x7d2756) || _0x7d2756.startsWith("\\\\")) {
      return `file://${_0x7d2756.replace(/\\/g, "/")}`;
    }
...
function createInitialProject(initialSettings = {}) {
...
export function createReplacementStudioApplication({
  ...
  let _0x479b40 = createInitialProject(initialSettings);
  ...
  const _0x4ff489 = () => {
    if (isPersistable(_0x479b40)) {
      _0x1a8fc5();
      _0x28b128.replace(createInitialProject(initialSettings), {
        persist: false,
        presentation: "none",
        reason: "fresh-import",
  ...
  async function _0x168bc2({
    ...
    try {
      ...
      _0x5cae27({
        ...
        // 处理完成后必须刷新原始工作区，否则状态已写入项目但界面仍显示
        // 旧的“0 个片段/暂无素材”，用户会误以为上传或处理失败。
        renderWorkspace: true
      });
  ...
  return Object.freeze({
    ...
    refresh() {
      return _0x37df20?.refresh?.() ?? false;
    },
    isSourceProcessing() {

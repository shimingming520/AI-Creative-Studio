...
// 保持 ShuoCanvas 原始默认值；hosted 模式通过挂载层的 initialSettings
// 注入当前 YUH 中转站可用模型，避免影响独立运行的原始工作室。
...
export function resolvePersonReplacementVideoModelId(_0x5cef8b) {
  ...
  // hosted 模式下，模型菜单会注入 YUH 设置中的自定义中转模型，
  // 其内部 ID 采用 `custom-供应商ID/真实模型名`，不能被回退到 RunningHub。
  if (/^custom-[^/]+\/.+$/i.test(_0x2ad04e) || PERSON_REPLACEMENT_VIDEO_MODEL_IDS.includes(_0x2ad04e)) {
    return _0x2ad04e;
  } else {
